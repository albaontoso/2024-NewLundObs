...
angular_slices = [r'$0 < \eta < 1$',r'$1 < \eta < 2$', r'$2 < \eta < 3$', r'$3 < \eta < 4$']
...
with PdfPages('exclusive-lund.pdf') as pdf:
    for f,s in zip(files,showers):
        ratio_xs = get_xsection(f,'eta_lnkt_w_coll').xsc/get_xsection(f,'eta_lnkt_wo_coll').xsc
        dataw    = get_data(f, 'eta_lnkt_w_coll')
        datawo   = get_data(f, 'eta_lnkt_wo_coll')
        ...
        for i, l, c in zip([0,1,2,3], angular_slices,colours):
        ...
        ax.legend(loc='best', edgecolor='white',borderpad=0.1, fontsize=12)
